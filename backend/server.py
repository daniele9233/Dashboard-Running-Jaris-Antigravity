...
# Per casi specifici (TRX Marathon livello 3/15 = 2.6% pendenza reale):
# il 2.6% compensa aerodinamica + nastro motorizzato (Jones&Doust 1996 +
# costo eccentrico), quindi l'effort sul tapis = effort strada piatta a
# parità di pace mostrato. Override: distanza/durata/pace corretti e
# is_treadmill=False per includere run in VDOT/zone/race predictions.
...
_MANUAL_RUN_OVERRIDES: dict[tuple[str, str], dict] = {
    ("Soglia Intensa Tapis Roulant", "2026-05-20"): {
        ...
        "avg_pace": "4:10",
        "is_treadmill": False,  # 2.6% incline compensa → road-equivalent
        "was_treadmill": True,  # informativo
        "treadmill_grade_pct": 2.6,
        ...
        "notes": (
            ...
            "per equivalente strada piatta. Struttura: "
            "2km@13.5kmh + 2km@14.5kmh + 1km@15.5kmh + 300m@16.5kmh."
        ),
    },
}
...
def _apply_manual_override(run_doc: dict) -> dict:
    ...
    Returns the (mutated) run_doc. Preserves avg_hr/max_hr/avg_cadence from
    source. Override fields take precedence.
    """
    ...
    # Ricalcola HR% se HR presente (max_hr profilo non disponibile qui →
    # recupera da run_doc.max_hr_pct se già impostato)
    return run_doc
...
async def strava_sync():
    ...
    async with httpx.AsyncClient(timeout=30.0) as http:
        ...
        for act in all_activities:
            ...
            # Manual overrides per casi specifici (es. treadmill TRX compensato).
            # Applicato PRIMA di GAP/intervals/normalize per usare i valori corretti.
...
async def reapply_manual_overrides():
    ...
    già sincronizzato (skip-existing nel sync normale).
    """
    tokens = await _get_active_strava_token()
    ...
    # Trigger ricalcolo CTL/ATL/TSB + auto-adapt zone
    if athlete_id:
        profile_doc = await db.profile.find_one({"athlete_id": athlete_id})
        max_hr_p = int(profile_doc.get("max_hr", 190)) if profile_doc else 190
        resting_hr_p = int(profile_doc.get("resting_hr", 50)) if profile_doc else 50
