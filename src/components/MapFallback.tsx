  ...
  return (
    <div className="rounded-3xl overflow-hidden h-full bg-[#0F172A] border border-white/[0.04] flex items-center justify-center">
      <div className="text-center p-6">
        <MapPin className="w-8 h-8 mx-auto mb-2 text-brand opacity-50" />
        <div className="text-brand text-3xl font-black mb-2">{run.distance_km.toFixed(1)} <span className="text-sm text-gray-500">km</span></div>
        ...
        <span className="text-[#94A3B8] text-[11px]">{formatDate(run.date)}</span>
      </div>
    </div>
  );
}