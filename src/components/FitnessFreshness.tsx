...
const CLR_TSB_POS = "#14B8A6";
const CLR_TSB_NEG = "#8B5CF6";
...
const FFTooltip = ({ active, payload, label }: any) => {
  ...
  const trimp = payload[0]?.payload?.trimp ?? null;
  const dateStr = label
  ...
  return (
    <div className="bg-[#0F172A] border border-[#334155] px-3 py-2.5 rounded-xl shadow-2xl text-xs min-w-[160px]">
      <p className="text-[#94A3B8] font-bold mb-2 tracking-wider">{dateStr}</p>
      <div className="space-y-1.5">
        <div className="flex justify-between gap-4">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: CLR_CTL }} />
            <span style={{ color: CLR_CTL }}>CTL</span>
          </span>
          <span className="text-white font-bold">{ctl?.toFixed(1) ?? "—"}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: CLR_ATL }} />
            <span style={{ color: CLR_ATL }}>ATL</span>
          </span>
          <span className="text-white font-bold">{atl?.toFixed(1) ?? "—"}</span>
        </div>
        <div className="flex justify-between gap-4">
          <span className="flex items-center gap-1.5">
            <span className="w-2 h-2 rounded-full inline-block" style={{ backgroundColor: tsbColor }} />
        ...
        {trimp != null && (
          <div className="border-t border-[#334155] pt-1.5 flex justify-between gap-4">
            <span className="text-[#475569]">TRIMP</span>
            <span className="text-[#475569]">{trimp.toFixed(0)}</span>
          </div>
        )}
      </div>
    </div>
    ...
    <div className="bg-bg-card border border-[#1E293B] rounded-xl p-6">
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-5">
          ...
          <div className="text-[10px] text-[#475569] mt-3">Acute Training Load · 7gg</div>
        </div>

        {/* TSB */}
        <div className="bg-[#0F172A] border border-[#1E293B] rounded-xl p-5" style={{ borderColor: statusColor + "30" }}>
          ...
          <div
            className="text-xs font-black uppercase tracking-widest mt-2 px-2 py-0.5 rounded inline-block"
      ...
      {fitnessFreshness && fitnessFreshness.length > 0 ? (
        <>
          {/* Legend */}
          <div className="flex gap-6 mb-3 text-[10px] font-semibold tracking-wider text-text-muted">
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-[2px] rounded-full" style={{ backgroundColor: CLR_CTL }} />
              <span style={{ color: CLR_CTL }}>CONDIZIONE (CTL)</span>
            </div>
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-[2px] rounded-full" style={{ backgroundColor: CLR_ATL }} />
              <span style={{ color: CLR_ATL }}>AFFATICAMENTO (ATL)</span>
            </div>
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[#94A3B8]">FORMA (TSB)</span>
            </div>
          </div>
          <div style={{ height: 240 }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={fitnessFreshness} margin={{ top: 8, right: 16, left: -8, bottom: 0 }}>
